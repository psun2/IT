...
var
	...
	jQuery = 함수 (선택자, 컨텍스트) {
		...
		// jQuery가 호출되면 init 필요 (포함되지 않은 경우 오류가 발생하도록 허용)
...
함수 Sizzle (선택자, 컨텍스트, 결과, 시드) {
	...
	// HTML 문서에서 찾기 작업을 바로 가기 시도 (필터와 반대)
	if (! seed) {
...
함수 createDisabledPseudo (disabled) {
	...
	리턴 함수 (elem) {
		...
		if (elem의 "form") {
			...
			// 비활성화되지 않은 관련 요소에서 상속 된 비활성화를 확인합니다.
...
setDocument = Sizzle.setDocument = 함수 (노드) {
	...
	// 지원 : IE 9-11, Edge
	// 언로드 후 iframe 문서에 액세스하면 "permission denied"오류가 발생 함 (jQuery # 13936)
	if (preferredDoc! == 문서 &&
	...
	Expr.find [ "CLASS"] = support.getElementsByClassName && function (className, 컨텍스트) {
		if (typeof context.getElementsByClassName! == "undefined"&& documentIsHTML) {
			컨텍스트 반환. getElementsByClassName (className);
		}
	};
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		assert (function (el) {
			...
			// 지원 : IE9-11 +
			// IE의 : disabled 선택기는 비활성화 된 필드 집합의 자식을 선택하지 않습니다.
